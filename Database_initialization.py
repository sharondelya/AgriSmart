...
DB_CONFIG = {
    ...
    'database': os.environ.get('DB_NAME', 'agrismart'),
...
def insert_admin_user(cursor):
    ...
    try:
        ...
        admin_email = os.environ.get('ADMIN_EMAIL', 'admin@honeybee')
        admin_name = os.environ.get('ADMIN_NAME', 'Admin')
        admin_password = os.environ.get('ADMIN_PASSWORD', 'change_this_password')
        
        # Check if admin user already exists
        cursor.execute("SELECT COUNT(*) FROM admin2 WHERE admin_email = %s", (admin_email,))
        ...
        if admin_count == 0:
            ...
            logger.info(f"✅ Default admin user '{admin_name}' created successfully")
            logger.info(f"   📧 Email: {admin_email}")
            logger.info("   🔐 Password: [Set via environment variable]")
        else:
